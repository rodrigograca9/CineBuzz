...
// ListCard component - Fixed version
  ...
  useEffect(() => {
    const processMovies = () => {
      try {
        ...
        if (list.movies && Array.isArray(list.movies) && list.movies.length > 0) {
          movies = list.movies;
        } else if (list.movie_ids && Array.isArray(list.movie_ids) && list.movie_ids.length > 0) {
          movies = list.movie_ids;
        } else {
          // If no movies array is found, check if it might be a string that needs parsing
          try {
            if (typeof list.movies === 'string' && list.movies) {
              const parsedMovies = JSON.parse(list.movies);
              if (Array.isArray(parsedMovies)) {
                movies = parsedMovies;
              }
            } else if (typeof list.movie_ids === 'string' && list.movie_ids) {
        ...
        // Filter out any null, undefined, or falsy values
...
const LoadingState = () => (
  <div className="bg-gradient-to-b from-slate-900 to-black text-white min-h-screen flex items-center justify-center">
    <div className="text-center">
      ...
      <p className="text-lg text-blue-200">A carregar listas de filmes...</p>
    </div>
  </div>
);
...
export default function UserLists() {
    ...
    const fetchData = async () => {
      try {
        ...
        const processedLists = (listsData || []).map(list => {
          ...
          // Handle different possible movie data formats
          ...
          // If still not an array or is null/undefined, initialize as empty array
          if (!Array.isArray(movieIds)) {
            movieIds = [];
          }
          
          // Filter out any null, undefined, or falsy values
          movieIds = movieIds.filter(id => id);
  ...
  onClick={async () => {
    ...
    if (error) {
      ...
      window.location.reload(); // ou idealmente: atualizar userLists sem reload
    }
  }}
