...
export default function UserWatchlist() {
  ...
  const handleRemoveFromWatchlist = async (movieId) => {
    ...
    try {
      ...
      const updatedWatchlist = userData.watchlist.filter(id => id !== movieId);
      ...
      if (error) throw new Error("Erro ao atualizar watchlist: " + error.message);
      ...
      console.error(err);
      alert("Erro ao remover filme da watchlist: " + err.message);
    }
  };
    ...
    return (
      <div className="bg-[#14181C] text-white min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg">Carregando watchlist...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-white">
                {userData.username ? `Watchlist de ${userData.username}` : 'Watchlist'}
              </h1>
            </div>
            <div className="bg-blue-800 px-4 py-2 rounded-full">
              <span className="font-medium">{userData.watchlist.length} {userData.watchlist.length === 1 ? 'filme' : 'filmes'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-12">
        {userData.watchlist.length === 0 ? (
          <div className="bg-[#1C2228] rounded-lg p-8 text-center">
            ...
            <p className="text-gray-400 text-lg">Nenhum filme na watchlist ainda.</p>
            <button 
              onClick={() => navigate('/')}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {userData.watchlist.map(movieId => (
              <MovieCard 
                ...
                movieId={movieId} 
                onNavigate={navigate} 
                onRemove={isOwner ? handleRemoveFromWatchlist : null}
              />
            ))}
          </div>
        )}
